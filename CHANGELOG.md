...
- **Import and export** your list of targets, from the `⋯` menu on Home. Export writes
  a signed `.lumenhop` file — title, host, icon and interval — that anyone can import.
  The importer verifies the file's signature and checksum, skips hosts you already have,
  and never carries over local-only data like custom icon files.
